      ...
      <div className="w-full md:w-36 h-36 bg-slate-900 border-b md:border-b-0 md:border-r border-white/5 shrink-0 relative flex items-center justify-center">
        {image_path ? (
          ...
          <div className="flex flex-col items-center justify-center text-slate-500">
            ...
            <span className="text-xs uppercase font-extrabold tracking-wider text-slate-500">No Image</span>
          </div>
        )}
      </div>
      ...
      <div className="p-5 flex-1 flex flex-col justify-between">
        <div>
          <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
            <span className="text-xs uppercase font-extrabold tracking-widest text-violet-400">Scan Analysis</span>
          ...
          <h3 className="text-xl font-bold text-white mb-2 leading-tight group-hover:text-violet-300">
            {condition || 'Analyzing...'}
          </h3>
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1.5 text-xs text-white/60 mb-3">
            {body_part && (
              <span className="flex items-center gap-1">
                ...
                Region: <strong className="text-white font-semibold">{body_part}</strong>
              </span>
            )}
            {skin_type && (
              <span className="flex items-center gap-1">
                <Layers className="w-3.5 h-3.5 text-white/40" />
                Skin: <strong className="text-white font-semibold">{skin_type}</strong>
              </span>
            )}
          </div>
          ...
          {symptoms && symptoms.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-3">
              {symptoms.slice(0, 3).map((sym, idx) => (
                <span key={idx} className="bg-white/5 text-xs font-semibold text-white/75 px-2.5 py-1 rounded-full">
                  {sym}
                </span>
              ))}
              {symptoms.length > 3 && (
                <span className="bg-white/5 text-xs font-semibold text-white/45 px-2 py-1 rounded-full">
